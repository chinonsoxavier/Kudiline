  ...
  return (
    <div className="center gap-4 flex-col max_width">
      <div className="center gap-1 rounded-full border-[0.5px] border-primary/5 glow-shadow py-1 px-4">
        ...
        <p className="text-lg font-semibold">Kudiline Steps</p>
      </div>

      <div className="grid-cols-5 py-6 text-[#0E0E0E] grid items-start w-full gap-3 flex-wrap justify-end">
        {steps.map((step, index) => (
          <div
                className={`rounded-[30px] p-6 min-h-[409px] h-full flex-1 ${step.span === 2 ? 'col-span-3' : 'col-span-2'}`}
          ...
          >
            ...
            >
              {index + 1}
                ...
                <div className={`flex relative py-4 items-start justify-between ${step.span===2 ? '' :''}`}>
              <div className="h-full">
                <p className="text-xl font-semibold leading-10">
                  {step.title}
                </p>

                <p className="text-lg text-foreground">{step.desc}</p>
              </div>

              <img
                src={step.icon}
                alt=""
                className="relative flex- w-[221px] top-16 object-cover max-w-[221px] right-0"
              />
            </div>
          </div>
        ))}
